...
export default defineEventHandler(async (event) => {
  const {
    ...
    public: { hubspotApiDomain, meetingSchedulerName, meetingSchedulerOrganizerId, meetingSchedulerType },
  ...
  const { success, error, data: query } = await getValidatedQuery(event, querySchema.safeParse)
  if (!success) throw error.issues

  const response = await $fetch<{
    total: number
    results: HubspotMeeting[]
  }>(new URL('/scheduler/v3/meetings/meeting-links', hubspotApiDomain).toString(), {
    query,
    headers: {
